...
##题目顺序
var type_order:Array=["填空题","单选题","多选题","判断题","简答题"]
...
var current_question:Dictionary:
	set(new):
		if current_question!=new:
			current_question=new
			question_changed.emit(current_question)

var config:Dictionary={"show_answer":false,
...
func _ready() -> void:
	match OS.get_name():
		...
		"Android":
			...
			json_save_path="/storage/emulated/0/Android/data/com.example.senergy/json_data"
		"iOS":
...
##初始化数据
func init_data(json_path:String):
	#清除缓存
	clear_data()
	#获取数据
	var json_content=load_json_file(json_path)[0]
	all_content=json_content
	#赋值
	for content in json_content:
		if not content.has("题型"):
			continue
		match content["题型"]:
			"填空题":
...
func to_new_type(current_order:int,type:String,is_plus:bool):
	...
	match type:
		"单选题":
			#获取当前题型的顺序
			var type_index:int=type_order.find("单选题")
			#若题号小于等于一，并且是去上一题
			if current_order<=1 and not is_plus:
				...
				if type_index==0:
					return 0
				#若该题型顺序不为0，则进入上一个题型
				to_certain_question(type_order[type_index-1])
				#代表已经执行跳转，各场景不要展示数据了
				return 1
			#若该题号小于
			
			elif current_order>=single_content.size() and is_plus:
				if type_index==type_order.size()-1:
					return 0
				to_certain_question(type_order[type_index+1])
				return 1
		"多选题":
			var type_index:int=type_order.find("多选题")
			if current_order<=1 and not is_plus:
				if type_index==0:
					return 0
				to_certain_question(type_order[type_index-1])
				...
				return 1
		"简答题":
			var type_index:int=type_order.find("简答题")
			if current_order<=1 and not is_plus:
				if type_index==0:
					return 0
				to_certain_question(type_order[type_index-1])
				...
				return 1
		"判断题":
			var type_index:int=type_order.find("判断题")
			if current_order<=1 and not is_plus:
				if type_index==0:
					return 0
				to_certain_question(type_order[type_index-1])
		...
		"填空题":
			#print(fill_content)
			var type_index:int=type_order.find("填空题")
			if current_order<=1 and not is_plus:
				if type_index==0:
					return 0
				to_certain_question(type_order[type_index-1])
...
##获取该题目的题号
func get_question_order(que:Dictionary):
	return get_question_type_array(que.题型).find(que)+1

##获取当前题目的题号
func get_current_question_order():
	if not current_question:
		printerr("当前题目为空，无法获取当前题型。位于Global脚本"+\
		"get_current_question_order()函数")
	return get_current_question_type_array().find(current_question)+1

##获取该类型的题库数组
func get_question_type_array(type:String)->Array:
	match type:
		"填空题":
			return fill_content
		"单选题":
...
##获取当前类型题目的题库数组
func get_current_question_type_array()->Array:
	if not current_question:
		printerr("当前题目为空，无法获取当前题型。位于Global脚本get_current_question_type_array()函数")
		return []
	match current_question.题型:
		"填空题":
			return fill_content
		"单选题":
			return single_content
...
##获取该问题的下一个题型
func get_next_type(que:Dictionary)->String:
	#判断是否为最后一个题型
	if type_order.find(que.题型)+1==type_order.size():
		print("已经是最后一个题型了:",que.题型)
		return ""
	#示例：选择题
	print(type_order[type_order.find(que.题型)+1])
	return type_order[type_order.find(que.题型)+1]

##获取当前问题的下一个题型
func get_current_next_type()->String:
	return get_next_type(current_question)
	
##获取该问题的上一个题型
func get_last_type(que:Dictionary)->String:
	#判断是否是第一个题型
	if type_order.find(que.题型)==0:
		#print(type_order.find(que.题型)+1)
		#print(type_order.size())
		print("已经是第一个题型了:",que.题型)
		return ""
	#示例：选择题
	return type_order[type_order.find(que.题型)-1]

##获取当前问题的下一个题型
func get_current_last_type()->String:
	return get_last_type(current_question)
